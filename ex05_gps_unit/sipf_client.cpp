...
#define tag_N 3
static uint8_t _tag_id[tag_N] = {0x01,0x02,0x03};
static SipfObjTypeId _type[tag_N] = {OBJ_TYPE_UINT8,OBJ_TYPE_UINT8,OBJ_TYPE_UINT8};
static uint8_t *_value[tag_N];
static uint8_t _value_len[tag_N] = {0,0,0};

void SipfCmdTxN(uint8_t n, uint8_t tag_id, SipfObjTypeId type, uint8_t *value, uint8_t value_len)
{
	if(n == tag_N) SipfCmdTx3(tag_id, type, value, value_len, NULL);
	if(n >= tag_N) return;
	n--;
	_tag_id[n] = tag_id;
	_type[n] = type;
	_value[n] = value;
	_value_len[n] = value_len;
}

void SipfCmdTx1(uint8_t tag_id, SipfObjTypeId type, uint8_t *value, uint8_t value_len)
{
	SipfCmdTxN(1,tag_id, type, value, value_len);
}

void SipfCmdTx2(uint8_t tag_id, SipfObjTypeId type, uint8_t *value, uint8_t value_len)
{
	SipfCmdTxN(2,tag_id, type, value, value_len);
}

int SipfCmdTx3(uint8_t tag_id, SipfObjTypeId type, uint8_t *value, uint8_t value_len, uint8_t *otid)
{
    int len;
    int ret;
    ...
    // $$TXコマンド送信
    len = sprintf(cmd, "$$TX");
	for(int n = 0; n < tag_N; n++){
    	len += sprintf(&cmd[len], " %02X %02X ", _tag_id[n], (uint8_t)_type[n]);
	    switch (_type[n]) {
	        case OBJ_TYPE_BIN:
	        case OBJ_TYPE_STR_UTF8:
				//順番どおりに文字列に変換
	            for (int i = 0; i < _value_len[n]; i++) {
	                len += sprintf(&cmd[len], "%02X", _value[n][i]);
	            }
	            break;
	        default:
	            // リトルエンディアンだからアドレス上位から順に文字列に変換
	            for (int i = (_value_len[n] - 1); i >= 0; i--) {
	                len += sprintf(&cmd[len], "%02X", _value[n][i]);
	            }
	            break;
	    }
    ...
    for (;;) {
        ...
        if (ret == 33) {
            //OTIDらしきもの
            if(otid != NULL) memcpy(otid, cmd, 32);
            break;
        }
        if (memcmp(cmd, "NG", 2) == 0) {
            //NG
