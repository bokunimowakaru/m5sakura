...
static uint8_t _tag_id1 = 0x01;
static SipfObjTypeId _type1 = OBJ_TYPE_UINT8;
static uint8_t *_value1;
static uint8_t _value_len1 = 0;
...
void SipfCmdTx1(uint8_t tag_id, SipfObjTypeId type, uint8_t *value, uint8_t value_len)
{
	...
	_type1 = type;
	_value1 = value;
	_value_len1 = value_len;
}

int SipfCmdTx2(uint8_t tag_id2, SipfObjTypeId type2, uint8_t *value2, uint8_t value_len2, uint8_t *otid)
{
    int len;
    int ret;
    ...
    // $$TXコマンド送信
    len = sprintf(cmd, "$$TX %02X %02X ", _tag_id1, (uint8_t)_type1);
    switch (_type1) {
        case OBJ_TYPE_BIN:
        case OBJ_TYPE_STR_UTF8:
			//順番どおりに文字列に変換
            for (int i = 0; i < _value_len1; i++) {
                len += sprintf(&cmd[len], "%02X", _value1[i]);
            }
            break;
        default:
            // リトルエンディアンだからアドレス上位から順に文字列に変換
            for (int i = (_value_len1 - 1); i >= 0; i--) {
                len += sprintf(&cmd[len], "%02X", _value1[i]);
            }
            break;
    }
    len += sprintf(&cmd[len], " %02X %02X ", tag_id2, (uint8_t)type2);
    switch (type2) {
        case OBJ_TYPE_BIN:
        case OBJ_TYPE_STR_UTF8:
			//順番どおりに文字列に変換
            for (int i = 0; i < value_len2; i++) {
                len += sprintf(&cmd[len], "%02X", value2[i]);
            }
            break;
        default:
            // リトルエンディアンだからアドレス上位から順に文字列に変換
            for (int i = (value_len2 - 1); i >= 0; i--) {
                len += sprintf(&cmd[len], "%02X", value2[i]);
            }
            break;
    }
